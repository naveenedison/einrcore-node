...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'einrcore-lib': '^' + bitcore.version,
    'einrcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/einrcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the einrcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the einrcore-node directory and configuration
