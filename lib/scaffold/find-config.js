...
/**
 * Will return the path and einrcore-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'einrcore-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'einrcore-node.json'))
  };
}
