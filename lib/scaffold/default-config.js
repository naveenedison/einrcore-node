...
/**
 * Will return the path and default einrcore-node configuration. It will search for the
 * configuration file in the "~/.einrcoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.einrcoin');
  var defaultConfigFile = path.resolve(defaultPath, './einrcore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/einrcoind')
          }
        }
      }
    };
